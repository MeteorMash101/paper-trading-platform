...
def getTestUser():
    return Account.objects.filter(email = "test@gmail.com")[0]

def authReturnValid(isValid, name="Will", email="test@gmail.com", google_user_id="test"):
    if isValid:
        return {
            "result": {
                "token": "valid",
            ...
            "data": {
                "name": name, 
                "email": email, "google_user_id": google_user_id
            }
        }
    else:
        return {
...
class StockListTestCases(TestCase):
    ...
    def buyOrSellStock(self, action, symbol, quantity, price = None):
        ...
        if price == None:   #If price isn't given cuz like if we change it idk seems smart
            return Client().put(url, {"action": action, "stock": symbol, "quantity":quantity, "access_token": "auth_is_valid"}, content_type="application/json")
        else:
            return Client().put(url, {"action": action, "stock": symbol, "quantity":quantity, "marketPrice": price, "access_token": "auth_is_valid"}, content_type="application/json")
    ...
    @patch("accounts.views.exchange_token")
    @patch('stocks.financeAPI.Stock_info.get_live_price')
    @mock.patch('accounts.utils.transactionHandler.datetime', FakeDate)
    def test_buy_share_not_owned_raw_data(self, livePriceAPI, token_auth):
        """Test buying a user's first stock"""
        token_auth.return_value = authReturnValid(True)
        createTestUser()
    ...
    @patch('stocks.financeAPI.Stock_info.get_live_price')
    @mock.patch('accounts.utils.transactionHandler.datetime', FakeDate)
    def test_buy_share_transaction_history(self, livePriceAPI, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        stockPrice = 123.50
        FakeDate.today = classmethod(lambda cls: datetime(2022, 2, 15))
    ...
    @patch('stocks.financeAPI.Stock_info.get_live_price')
    def test_valid_balance_after_purchase(self, livePriceAPI, token_auth):
        ...
        user = getTestUser()
        self.assertEqual(float(user.balance) + stockPrice*quantity, 100000.00)

    @patch("accounts.views.exchange_token")
    #Buy the same stock back to back
    @patch('stocks.financeAPI.Stock_info.get_live_price')
    @mock.patch('accounts.utils.transactionHandler.datetime', FakeDate)
    def test_buy_share_some_owned_raw_data(self, livePriceAPI, token_auth):
    ...
    @patch("accounts.views.exchange_token")
    @patch('stocks.financeAPI.Stock_info.get_live_price')
    def test_sell_one_share_one_owned(self, livePriceAPI, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        ...
        self.assertEqual(100000+3*(123.77-123.50), float(user.balance))

    @patch("accounts.views.exchange_token")
    @patch('stocks.financeAPI.Stock_info.get_live_price')
    @mock.patch('accounts.utils.transactionHandler.datetime', FakeDate)
    def test_sell_shares_single_purchase(self, livePriceAPI, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        (buyPrice, sellPrice) = (123.50, 123.77)
    ...
    @patch('stocks.financeAPI.Stock_info.get_live_price')
    @mock.patch('accounts.utils.transactionHandler.datetime', FakeDate)
    def test_sell_shares_multiple_purchase(self, livePriceAPI, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
    ...
    @patch("accounts.views.exchange_token")
    def test_number_of_shares_none_owned(self, token_auth):
        """Checking that it returns 0 when the stock doesn't exist"""
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        url = reverse("accounts:ownedStockList", args=("test", ))
        data = Client().get(url, data = {"info": "num_of_ticker_stocks", "symbol":"dis", "access_token": "auth is valid"}, content_type="application/json").json()
        self.assertEqual(data["quantity_owned"], 0)

    @patch("accounts.views.exchange_token")
    def test_number_of_shares_one_owned(self, token_auth):
    ...
    @patch("accounts.views.exchange_token")
    def test_number_of_shares_many_owned(self, token_auth):
    ...
    def test_stock_list_when_none_owned(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()

        url = reverse("accounts:ownedStockList", args=("test", ))
        data = Client().get(url, data = {"info": "stock_list_display", "access_token": "auth is valid"}, content_type="application/json").json()
        self.assertTrue(len(data["stock_list"]) == 0)

    @patch("accounts.views.exchange_token")
    @patch('stocks.financeAPI.Stock_info.get_price_and_change_for_list')
    def test_stock_list_when_one_owned(self, priceAndChangeAPI, token_auth):
    ...
    @patch("accounts.views.exchange_token")
    @patch('stocks.financeAPI.Stock_info.get_price_and_change_for_list')
    def test_stock_list_when_many_owned(self, priceAndChangeAPI, token_auth):
    ...
    ###########################################################################
    @patch("accounts.views.exchange_token")
    def test_portfolio_value_no_stocks(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        url = reverse("accounts:ownedStockList", args=("test", ))
        data = Client().get(url, data = {"info": "portfolio_value", "access_token": "auth is valid"}, content_type="application/json").json()
        expected = {"portfolio_value": "100000.00", "percent_change": '0.00', "change_direction": False}
        self.assertEqual(data, expected)
    
    @patch("accounts.views.exchange_token")
    @patch('stocks.financeAPI.Stock_info.get_price_and_change_for_list')
    def test_portfolio_value_one_share(self, livePriceAPI, token_auth):
        #Set up the fake API
        stockPrice = 2684.99
        ...
        #Test
        url = reverse("accounts:ownedStockList", args=("test", ))
        pv = Client().get(url, data = {"info": "portfolio_value", "access_token": "auth is valid"}, content_type="application/json").json()
        expected = {"portfolio_value": str(100000 + stockPrice), "percent_change": str(round(100*(stockPrice - buyPrice)/buyPrice, 2)), 'change_direction': (stockPrice - buyPrice)/stockPrice > 0}
        self.assertEqual(pv, expected)
    
    @patch("accounts.views.exchange_token")
    @patch('stocks.financeAPI.Stock_info.get_price_and_change_for_list')
    def test_portfolio_value_many_stocks(self, livePriceAPI, token_auth):
        #Set up the fake API call
        ...
        expected = {"portfolio_value": str(100000 + expectedPrice), "percent_change": str(pvChange), 'change_direction': pvChange > 0}
        self.assertEqual(pv, expected)
    ...
    def test_portfolio_value_history_generation_from_start_date(self, token_auth):
        ...
        Client().get(url, {"access_token": "auth is valid"})
        user = getTestUser()
    ...
    @mock.patch('accounts.utils.historicalPortfolioValueLoader.date', FakeDate)
    @mock.patch('accounts.utils.historicalPortfolioValueLoader.PortfolioValue.load', FakeAPI.portfolioHistory)
    def test_portfolio_value_history_update(self, token_auth):
    ...
    def test_user_performance_api(self, token_auth):
        ...
        data = Client().get(url, {"access_token": "auth is valid"}).json()
        expected = [
            {'date': '2022-05-10', 'value': 10000000.0, 'value_change': 0.0, 'percent_change': 0.0, 'change_direction': False},
            {'date': '2022-05-11', 'value': 9972089.0, 'value_change': -27911.0, 'percent_change': -0.27911, 'change_direction': False}, 
            {'date': '2022-05-12', 'value': 9933167.0, 'value_change': -66833.0, 'percent_change': -0.66833, 'change_direction': False}, 
            {'date': '2022-05-13', 'value': 9999432.0, 'value_change': -568.0, 'percent_change': -0.00568, 'change_direction': False}, 
            {'date': '2022-05-14', 'value': 9999432.0, 'value_change': -568.0, 'percent_change': -0.00568, 'change_direction': False}, 
            {'date': '2022-05-15', 'value': 9999432.0, 'value_change': -568.0, 'percent_change': -0.00568, 'change_direction': False}, 
            {'date': '2022-05-16', 'value': 10244719.0, 'value_change': 244719.0, 'percent_change': 2.44719, 'change_direction': True}, 
            {'date': '2022-05-17', 'value': 10381131.0, 'value_change': 381131.0, 'percent_change': 3.81131, 'change_direction': True}
        ]
    ...
    ###########################################################################
    @patch("accounts.views.exchange_token")
    def test_raw_stock_list_when_none_owned(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        url = reverse("accounts:ownedStockList", args=("test", ))
        ownedStocks = Client().get(url, data = {"info": "stock_list_detailed", "access_token": "auth is valid"}, content_type="application/json").json()
        self.assertEqual(ownedStocks, {"stock_list": {}})

    @patch("accounts.views.exchange_token")
    def test_raw_stock_list_many_owned(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        #Create fake user data
        createTestUser()
        ...
        #Test
        url = reverse("accounts:ownedStockList", args=("test", ))
        ownedStock = Client().get(url, data = {"info": "stock_list_detailed", "access_token": "auth is valid"}, content_type="application/json").json()
...
'''
    @patch("accounts.views.exchange_token")
    def test_see_nonexistent_account_details(self, token_auth):
    data = {"access_token": "auth is valid"}
    token_auth.return_value = authReturnValid(True, name="notPresent", email="notPresent", google_user_id="notPresent")
'''
###############################################################################
class WatchListTestCases(TestCase):
    ...
    @patch("accounts.views.exchange_token")
    def test_adding_ticker(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        ticker = "F"
        url = reverse("accounts:watchList", args=("test", ))
        Client().put(url, data = {"symbol": ticker, "access_token": "auth is valid"}, content_type="application/json")
    ...
    @patch("accounts.views.exchange_token")
    def test_removing_ticker(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        ticker = "F"
        user.watchList = {"stocks": [ticker]}
        user.save()
    ...
    @patch("accounts.views.exchange_token")
    def test_adding_multiple_tickers(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        tickers = ["F", "AAPL", "goog", "tsla", "V"]
        url = reverse("accounts:watchList", args=("test", ))
        for ticker in tickers:
            Client().put(url, data = {"symbol": ticker, "access_token": "auth_is_valid"}, content_type="application/json")
    ...
    @patch("accounts.views.exchange_token")
    def test_adding_removing_multiple_tickers(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
    ...
    @patch("accounts.views.exchange_token")
    def test_check_if_existing_stock_in_watchlist(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        ...
        url = reverse("accounts:watchList", args=("test", ))
        result = Client().get(url, data = {"info": "check_stock", "symbol": "F", "access_token": "auth is valid"}, content_type="application/json").json()
        self.assertTrue(result["isPresent"])

    @patch("accounts.views.exchange_token")
    def test_check_if_nonexistent_stock_in_watchlist(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        user.watchList = {"stocks": ["F", "V", "GOOG"]}
        ...
        url = reverse("accounts:watchList", args=("test", ))
        result = Client().get(url, data = {"info": "check_stock", "symbol": "TSLA", "access_token": "auth is valid"}, content_type="application/json").json()
    ...
    @patch("accounts.views.exchange_token")
    def test_get_all_watched_stocks_when_none(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        stockList = []
    ...
    @patch("accounts.views.exchange_token")
    def test_get_all_watched_stocks_when_one(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        stockList = ["GOOG"]
        ...
        url = reverse("accounts:watchList", args=("test", ))
        result = Client().get(url, data = {"info": "stocks", "access_token": "auth is valid"}, content_type="application/json").json()
        self.assertEqual(stockList, result["stock_list"])

    @patch("accounts.views.exchange_token")
    def test_get_all_watched_stocks_when_many(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        stockList = ["F", "V", "GOOG"]
    ...
    @patch("accounts.views.exchange_token")
    def test_get_detailed_watchlist_stock_info_when_none(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        stockList = []
        ...
        url = reverse("accounts:watchList", args=("test", ))
        result = Client().get(url, data = {"info": "detailed_stocks", "access_token": "auth is valid"}, content_type="application/json").json()
        self.assertEqual(result["stock_list"], [])

    @patch("accounts.views.exchange_token") 
    @patch('stocks.financeAPI.Stock_info.get_price_and_change_for_list')
    def test_get_detailed_watchlist_stock_info_when_one(self, priceAndChangeAPI, token_auth):
        #Set up the fake API call
        stockPrice = 14.39
        percentChange = 3.33173
    ...
    @patch("accounts.views.exchange_token")    
    @patch('stocks.financeAPI.Stock_info.get_price_and_change_for_list')
    def test_get_detailed_watchlist_stock_info_when_many(self, priceAndChangeAPI, token_auth):
...
class SimpleStockTestCases(TestCase):
    ...
    @patch("accounts.views.exchange_token")
    def test_transaction_history(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        createTestUser()
        user = getTestUser()
        ...
        history = Client().get(url, data=data).json()["transaction_history"]
        self.assertEqual(history, expected)

    @mock.patch('accounts.views.datetime', FakeDate)
    @patch("accounts.views.exchange_token")
    def test_reset(self, token_auth):
        token_auth.return_value = authReturnValid(True)
    ...
    def test_stock_diversity(self, industryAPI, token_auth):
        ...
        data = Client().get(url, data=data, content_type="application/json").json()
...
class GeneralAccountTestCases(TestCase):
    ...
    @mock.patch('accounts.views.datetime', FakeDate)
    @patch("accounts.views.exchange_token")
    def test_create_account(self, token_auth):
        token_auth.return_value = authReturnValid(True)
        FakeDate.today = classmethod(lambda cls: datetime(2022, 5, 28))
        ...
        expected = {"name": "Will", "email": "test@gmail.com", "google_user_id": "test"}
        self.assertEqual(user.name, expected["name"])
        self.assertEqual(user.email, expected["email"])
        self.assertEqual(user.google_user_id, expected["google_user_id"])
        self.assertEqual(user.balance, 100000)
        self.assertEqual(user.ownedStocks, {})
    ...
    @patch("accounts.views.exchange_token")
    def test_create_already_existing_account(self, token_auth):
        data = {"access_token": "auth is valid"}
        token_auth.return_value = authReturnValid(True)
        url = reverse("accounts:create")
        #data = {"name": "will", "email": "wckawamoto@ucdavis.edu", "google_user_id": "test"}
    ...
    def test_see_all_accounts_when_one(self):
        userData = {"user": None, "name": "Will", "email": "test@gmail.com", "google_user_id": "test",
                    "balance": "100000.00", "portfolio_value": "0.00", "start_date": "2022-02-01"}
    ...
    def test_see_all_accounts_when_many(self):
        userData = [{"user": None, "name": "Will", "email": "test1@gmail.com", "google_user_id": "test1",
                    "balance": "100000.00", "portfolio_value": "0.00", "start_date": "2022-02-15"},
                    ...
                    "balance": "100000.00", "portfolio_value": "0.00", "start_date": "2022-02-17"},
                    {"user": None, "name": "Willy", "email": "test3@gmail.com", "google_user_id": "test3",
                    "balance": "100000.00", "portfolio_value": "0.00", "start_date": "2022-02-26"},
                    {"user": None, "name": "Bill", "email": "test4@gmail.com", "google_user_id": "test4",
                    "balance": "100000.00", "portfolio_value": "0.00", "start_date": "2022-03-01"}]
    ...
    @patch("accounts.views.exchange_token")
    def test_see_existing_account_details(self, token_auth):
        data = {"access_token": "auth is valid"}
        
        userData = [{"user": None, "name": "Will", "email": "test1@gmail.com", "google_user_id": "test1",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": "2022-02-15"},
                    {"user": None, "name": "William", "email": "test2@gmail.com", "google_user_id": "test2",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": "2022-02-15"},
                    {"user": None, "name": "Billiam", "email": "test5@gmail.com", "google_user_id": "test5",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": "2022-02-17"},
                    {"user": None, "name": "Willy", "email": "test3@gmail.com", "google_user_id": "test3",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": "2022-02-26"},
                    {"user": None, "name": "Bill", "email": "test4@gmail.com", "google_user_id": "test4",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": "2022-03-01"}] 

        userToFind = userData[3]
        token_auth.return_value = authReturnValid(True, name=userToFind["name"], email=userToFind["email"], google_user_id=userToFind["google_user_id"])
        #Load data into database
        for acctInfo in userData:
        ...
        data = Client().get(url, data=data).json()
        self.assertEqual(data, userToFind)

    @patch("accounts.views.exchange_token")
    def test_see_nonexistent_account_details(self, token_auth):
        userData = [{"user": None, "name": "Will", "email": "test1@gmail.com", "google_user_id": "test1",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": None},
                    {"user": None, "name": "William", "email": "test2@gmail.com", "google_user_id": "test2",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": None},
                    {"user": None, "name": "Billiam", "email": "test5@gmail.com", "google_user_id": "test5",
                    "balance": "100000.00", "portfolio_value": "100000.00", "start_date": None},
