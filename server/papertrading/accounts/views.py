...
class AccountStocksOwned(APIView):
    ...
    def get(self, request, goog_id):
        ...
        if account != None:
            if data["info"] == "stock_list_detailed":            #Returns list of all their transactions with stocks they currently own.
                ...
                return Response(serializer.data)
            elif data["info"] == "buying_power_history":
    ...
    def purchaseStock(self, data, account):
        ...
        account.balance -= Decimal.from_float(data["quantity"]*si.get_live_price(data["stock"]))
        return True
    ...
    def calculatePortfolioChange(self, owned, curVal):
        ...
        for purchases in owned.values():
            for purchase in purchases:
                totalSpent += purchase["quantity"] * purchase["purchasePrice"]
